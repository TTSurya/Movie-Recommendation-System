...
Given only the observed entries `Ω`, the goal is to recover a complete matrix `X` that agrees with the known ratings:

$$
X_{ij} = M_{ij}, \qquad (i,j) \in \Omega.
$$
...
the singular values are transformed according to
...
followed by an update on the observed entries:
...
- The experiment uses the paper's synthetic setup: Gaussian low-rank factors, `τ = 5n`, and an oversampling ratio of 6.
...
1. **Operator check:** `D₃(diag(5, 2))` must equal `diag(2, 0)`, verifying the core singular-value shrinkage step.
...
For a `1000 × 1000` rank-10 synthetic matrix with an oversampling ratio of 6, the implementation converged in 121 iterations, recovered rank 10, and achieved a relative reconstruction error of approximately `1.70 × 10⁻⁴`.
